...
public class ListUtil {
    /**
     * @ 将相关字典表查询list信息合并到主业务查询数据List中(list数据强关联,mainList中存在匹配到dictList的数据才会返回) @param mainList
     *   主业务数据的list
    ...
    public static List<Map<String, Object>> mergeList(List<Map<String, Object>> mainList,
        ...
        for (Map<String, Object> mainMap : mainList) {
        	String mainValue = mainMap.get(mainKey.toLowerCase()) != null ? mainMap.get(mainKey.toLowerCase()).toString() : mainMap.get(mainKey.toUpperCase()).toString();
            ...
            for (Map<String, Object> dictMap : dictList) {
            	dictValue = dictMap.get(dictKey.toLowerCase()) != null ? dictMap.get(dictKey.toLowerCase()).toString() : dictMap.get(dictKey.toUpperCase()).toString();
                ...
                if (mainValue.equalsIgnoreCase(dictValue)) {
                    while (it.hasNext()) {
    ...
    /**
     * @ 将相关字典表查询list信息合并到主业务查询数据List中(list数据左关联,mainList中存在匹配到dictList的数据有值,其余的没值) @param mainList
     *   主业务数据的list
     * @param dictList 相关字典表信息list
     * @param mainKey 主业务数据中和字典信息关联的字段,主业务表中不含有该键值的数值,则直接返回主业务数据
     * @param dictKey 字典信息中和主业务数据关联的字段,字典表中不含有该键值的数值,则直接返回主业务数据

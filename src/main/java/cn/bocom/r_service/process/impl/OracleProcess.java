...
public class OracleProcess implements IProcess<String>{
    ...
    private static String DATE = "<if(flag)><col> between to_date('<time1>', 'yyyy-mm-dd HH24:mi:ss') and to_date('<time2>', 'yyyy-mm-dd HH24:mi:ss')<else><col> <oper> to_date('<time1>', 'yyyy-mm-dd HH24:mi:ss')<endif>";
    private static String SPLIT = "substr(<col>, (<if(flag)>0<else>instr(<col>, '<symbol>', 1, <leftIndex>) + 1<endif>), (decode(instr(<col>, '<symbol>', 1, <index>), 0, 0, instr(<col>, '<symbol>', 1, <index>) - <if(flag)>0<else>instr(<col>, '<symbol>', 1, <leftIndex>)<endif> - 1)))";
    ...
    public static void main(String[] q) {
    	//OracleProcess m = new OracleProcess();
    ...
    public String changeType(String col, String data, String newType) {
		...
		case "INT":
		    ...
		    break;
		case "FLOAT":
			col = "round(to_number(" + col + "),2)";
			break;
		case "DATE":
			col = "to_date(" + col + ", 'yyyy-mm-dd')";
			break;
		case "DATETIME":
			col = "to_date(" + col + ", 'yyyy-mm-dd HH24:mi:ss')";
			break;
		default:
			col = "to_char(" + col + ")";
			break;
